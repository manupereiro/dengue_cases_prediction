...
def get_department_id(department_name, province_name):
    """
    Obtiene el ID estandarizado basado en el nombre del departamento y provincia.
...
def process_dengue_data(input_file, output_file=None):
    """
    Procesa los datos de dengue/zika del formato original al formato requerido para el modelo.
    ...
    # Aplicar el mapeo de IDs estandarizados
    result['departament_id'] = result.apply(
        lambda row: get_department_id(row['departament_name'], row['province']), 
        axis=1
    )
    ...
    print(f"\nAños incluidos: {sorted(result['year'].unique())}")
...
if __name__ == "__main__":
    # Procesar el archivo de 2018
    input_file = "data/dengue_not_processed/dengue-zika.2018-12-31.csv"
    output_file = "data/dengue_processed_2018.csv"
    
    result_df = process_dengue_data(input_file, output_file)
